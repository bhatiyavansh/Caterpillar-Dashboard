...
export function Truck({ telemetry }: { telemetry: MachineTelemetry }) {
  ...
  const geo = useMemo(() => ({ side: profileGeometry(BODY_SIDE, 0.12, 0.02) }), []);
  ...
  useFrame(() => {
    ...
    if (body.current) body.current.rotation.x = Math.min(Math.max(-t.bucketAngle, 0), 1) * 0.95;
    if (heap.current) {
    ...
    <group ref={root}>
      <group ref={tilt}>
        ...
        <group ref={tractor} position={[0, 0, HITCH_Z]}>
          ...
          <RoundedBox args={[1.95, 1.25, 2.3]} radius={0.14} position={[0, 1.98, -3.45]} material={MAT.paint} castShadow receiveShadow />
          ...
          <Decal text="CAT" logo position={[0.99, 2.05, -3.5]} rotation={[0, Math.PI / 2, 0]} width={1.1} />
          <Decal text="CAT" logo position={[-0.99, 2.05, -3.5]} rotation={[0, -Math.PI / 2, 0]} width={1.1} />
          ...
          <group position={[0, 2.95, -1.55]}>
            ...
            <RoundedBox args={[2.1, 0.16, 1.8]} radius={0.05} position={[0, 0.86, 0]} material={MAT.paint} castShadow />
          ...
          {/* hitch */}
          <mesh position={[0, 1.05, 0]} rotation={[0, 0, 0]} material={MAT.steelDark} castShadow>
        ...
        <group ref={trailer} position={[0, 0, HITCH_Z]}>
          ...
          {[-1, 1].map((side) => (
            <mesh key={side} position={[side * 0.85, 1.0, 4.0]} material={MAT.steelDark} castShadow>
          ...
          <TailLamps telemetry={telemetry} positions={[[-1.2, 1.35, 5.62], [1.2, 1.35, 5.62]]} />

          {/* ---------------- dump body, hinged at the tail ---------------- */}
          <group ref={body} position={[0, 1.55, 5.2]}>
            <mesh geometry={geo.side} position={[1.62, 0, 0]} material={MAT.paint} castShadow receiveShadow />
            <mesh geometry={geo.side} position={[-1.62, 0, 0]} material={MAT.paint} castShadow receiveShadow />
            <Decal text="CAT" logo position={[1.7, 1.05, -2.3]} rotation={[0, Math.PI / 2, 0]} width={2.0} />
            <Decal text="CAT" logo position={[-1.7, 1.05, -2.3]} rotation={[0, -Math.PI / 2, 0]} width={2.0} />
            <Decal text="745" position={[1.7, 0.55, -4.3]} rotation={[0, Math.PI / 2, 0]} width={0.9} />
            {/* floor, rising to the headboard */}
            <mesh position={[0, 0.2, -2.4]} rotation={[-0.08, 0, 0]} material={MAT.paintWorn} castShadow receiveShadow>
              <boxGeometry args={[3.3, 0.14, 5.1]} />
            </mesh>
            {/* tail chute lip */}
            <mesh position={[0, 0.15, 0.55]} rotation={[0.2, 0, 0]} material={MAT.paintWorn} castShadow>
              <boxGeometry args={[3.3, 0.12, 1.2]} />
            </mesh>
            {/* headboard with canopy over the hitch */}
            <mesh position={[0, 1.3, -5.25]} rotation={[-0.08, 0, 0]} material={MAT.paint} castShadow>
            ...
            {[-1.0, -2.2, -3.4, -4.4].map((z) =>
              [-1, 1].map((side) => (
                <mesh key={`${z}${side}`} position={[side * 1.7, 0.95, z]} material={MAT.paintDark}>
                  <boxGeometry args={[0.06, 1.5, 0.1]} />
                </mesh>
              )),
            )}
            {/* the load */}
            <mesh ref={heap} position={[0, 0.55, -2.5]} material={MAT.dirt} visible={false} castShadow>
