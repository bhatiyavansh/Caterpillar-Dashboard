  ...
  return (
    <group ref={root}>
      <group ref={tilt}>
        ...
        <group ref={front}>
          {[-1, 1].map((side) => (
            <group key={side} position={[side * TRACK, TYRE_R, -AXLE]}>
              <Tyre radius={TYRE_R} width={TYRE_W} spinRef={spinRef} side={side as 1 | -1} />
            </group>
          ))}
          <mesh position={[0, 1.05, -1.3]} material={MAT.paint} castShadow receiveShadow>
          ...
          {/* ---------------- lift arms ---------------- */}
          <group ref={arms} position={[0, 2.05, -0.9]}>
            {[-0.62, 0.62].map((x) => (
              <mesh key={x} geometry={geo.arm} position={[x, 0, 0]} material={MAT.paint} castShadow />
            ))}
            {/* cross tube */}
            <mesh position={[0, -0.72, -1.9]} rotation={[0, 0, Math.PI / 2]} material={MAT.paint} castShadow>
            ...
            {/* ---------------- bucket ---------------- */}
            <group ref={bucket} position={[0, -1.45, -3.3]}>
              <mesh geometry={geo.shell} material={MAT.paintWorn} castShadow receiveShadow />
              <mesh geometry={geo.side} position={[1.45, 0, 0]} material={MAT.paintWorn} castShadow />
              <mesh geometry={geo.side} position={[-1.45, 0, 0]} material={MAT.paintWorn} castShadow />
              ...
              <mesh ref={load} position={[0, -0.25, -0.65]} material={MAT.dirt} visible={false} castShadow>
                <sphereGeometry args={[0.95, 12, 8, 0, Math.PI * 2, 0, Math.PI / 2]} />
              </mesh>
            </group>
          </group>
          <Ram from={liftBaseL} to={liftRodL} radius={0.1} />
          <Ram from={liftBaseR} to={liftRodR} radius={0.1} />
          <Ram from={tiltBase} to={tiltRod} radius={0.11} />
          {/* hitch */}
          <mesh position={[0, 1.0, 0]} material={MAT.steelDark} castShadow>
        ...
        <group ref={rear}>
          ...
          <mesh position={[0, 1.05, 1.4]} material={MAT.steelDark} castShadow>
            <boxGeometry args={[1.3, 0.7, 2.8]} />
          </mesh>
          {/* cab */}
          <RoundedBox args={[1.7, 0.3, 1.8]} radius={0.05} position={[0, 1.6, 0.75]} material={MAT.paint} castShadow />
          <group position={[0, 2.55, 0.75]}>
            ...
            <RoundedBox args={[1.7, 0.15, 1.75]} radius={0.05} position={[0, 0.86, 0]} material={MAT.paint} castShadow />
          ...
          <RoundedBox args={[1.9, 1.2, 2.3]} radius={0.16} position={[0, 1.95, 2.55]} material={MAT.paint} castShadow receiveShadow />
          <mesh position={[0, 2.0, 3.71]} material={MAT.grille}>
            <boxGeometry args={[1.5, 0.9, 0.04]} />
          </mesh>
          <Decal text="CAT" logo position={[0.97, 2.0, 2.55]} rotation={[0, Math.PI / 2, 0]} width={1.1} />
          <Decal text="CAT" logo position={[-0.97, 2.0, 2.55]} rotation={[0, -Math.PI / 2, 0]} width={1.1} />
          {/* counterweight bumper */}
          <RoundedBox args={[2.4, 0.9, 0.55]} radius={0.15} position={[0, 1.1, 3.95]} material={MAT.paint} castShadow />
          <Decal text="950" position={[0, 1.12, 4.23]} width={0.9} />
          <TailLamps telemetry={telemetry} positions={[[-0.95, 1.55, 4.22], [0.95, 1.55, 4.22]]} />
          {/* rear fenders */}
          {[-1, 1].map((side) => (
            <RoundedBox
              key={side}
