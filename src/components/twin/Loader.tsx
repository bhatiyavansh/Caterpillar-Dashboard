    ...
    <group ref={root}>
      <group ref={tilt}>
        ...
        {WHEEL_POSITIONS.map(([x, z]) => (
          <group key={`${x}:${z}`} ref={register} position={[x, 0.82, z]}>
        ...
        {/* cab */}
        <mesh position={[0, 2.25, 0.75]} castShadow>
        ...
        <mesh position={[0, 3.03, 0.75]} castShadow>
        ...
        <group ref={arms} position={[0, 1.15, -1.3]}>
          {[-1.05, 1.05].map((x) => (
            <mesh key={x} position={[x, -0.1, -1.3]} rotation={[-0.16, 0, 0]} castShadow>
              <boxGeometry args={[0.26, 0.34, 2.8]} />
              <meshStandardMaterial color={PALETTE.catYellow} roughness={0.6} metalness={0.3} />
            </mesh>
          ))}
          {/* tilt cylinder */}
          <mesh position={[0, 0.34, -1.0]} rotation={[-0.1, 0, 0]} castShadow>
