...
const LAYER_CONTROLS: { key: keyof TwinLayers; label: string; short: string; icon: LucideIcon }[] = [
  ...
  { key: "heatmap", label: "Incident density", short: "Incidents", icon: Flame },
      ...
      >
              ...
              {([
                ...
                <button
                  ...
                  className={cn(
                    ...
                    effectiveView === id ? "bg-cat-500 text-ink-950" : "text-muted hover:text-zinc-200",
                  )}
                >
        ...
        <div className="flex shrink-0 items-center justify-between gap-3 border-t border-white/10 bg-ink-900/70 px-3 py-1.5">
          ...
          <p className="hidden text-[10px] text-muted sm:block">
            {effectiveView === "3d" ? "Drag to orbit · scroll to zoom · click a machine to inspect" : "Click a machine to inspect it"}
