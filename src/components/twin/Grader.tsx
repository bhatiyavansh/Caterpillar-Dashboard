  ...
  return (
    <group ref={root}>
      <group ref={tilt}>
        ...
        {[-1, 1].map((side) => (
          <group key={side} ref={steerRef} position={[side * 1.08, TYRE_R, FRONT_Z]}>
        ...
        <group ref={circle} position={[0, 0.5, -1.2]}>
        ...
        <Ram from={liftBaseL} to={liftRodL} radius={0.07} />
        <Ram from={liftBaseR} to={liftRodR} radius={0.07} />
        ...
        {[-1, 1].map((side) => (
          <group key={side}>
            ...
            {TANDEM.map((z) => (
              <group key={z} position={[side * 1.08, TYRE_R, z]}>
                <Tyre radius={TYRE_R} width={TYRE_W} spinRef={spinRef} side={side as 1 | -1} />
              </group>
            ))}
            <RoundedBox
