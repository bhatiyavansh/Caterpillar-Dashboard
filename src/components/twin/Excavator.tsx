...
import { Anchor, Beacon, Decal, Lamp, MAT, Ram, profileGeometry, shoeTexture } from "./rig";
...
export function Excavator({ telemetry, safety = "safe", primary = false }: ExcavatorProps) {
  ...
  useFrame((state) => {
    ...
    if (primary && beacon.current) {
      ...
      const colour =
        safety === "critical" ? PALETTE.crit : safety === "warning" ? PALETTE.warn : PALETTE.catYellow;
  ...
  return (
    <group ref={root}>
      <group ref={tilt}>
        {/* ---------------- undercarriage ---------------- */}
        {[-1, 1].map((side) => (
          <TrackLoop key={side} side={side as -1 | 1} shoes={shoes} register={register} />
        ))}
        {/* car body tying the frames together */}
        <mesh position={[0, 0.72, 0]} material={MAT.steelDark} castShadow receiveShadow>
        ...
        <group ref={house} position={[0, 1.06, 0]}>
          {/* deck frame */}
          <mesh position={[0, 0.12, 0.3]} material={MAT.steelDark} castShadow receiveShadow>
            <boxGeometry args={[2.52, 0.24, 3.9]} />
          </mesh>

          {/* right-side tool box / pump compartment */}
          <RoundedBox args={[1.2, 0.95, 1.6]} radius={0.08} position={[0.64, 0.72, -0.5]} material={MAT.paint} castShadow receiveShadow />
          {/* engine hood, sloping to the rear */}
          <RoundedBox args={[2.46, 0.95, 1.55]} radius={0.12} position={[0, 0.72, 1.12]} material={MAT.paint} castShadow receiveShadow />
          <RoundedBox args={[2.3, 0.18, 1.35]} radius={0.06} position={[0, 1.26, 1.12]} material={MAT.paintDark} castShadow />
          ...
          <RoundedBox args={[2.54, 1.02, 0.78]} radius={0.3} smoothness={5} position={[0, 0.64, 2.2]} material={MAT.paint} castShadow receiveShadow />
          <Decal text="CAT" logo position={[0, 0.72, 2.6]} rotation={[0, 0, 0]} width={0.95} />
          <TailLights />
          ...
          <mesh position={[0.72, 1.9, 1.02]} rotation={[0.5, 0, 0]} material={MAT.black}>
          ...
          <Handrail from={[1.2, 1.55, -1.25]} to={[1.2, 1.55, 0.25]} posts={0.35} />
          <Handrail from={[1.0, 1.66, 0.55]} to={[1.0, 1.66, 1.75]} posts={0.3} />

          {/* boom foot bracket on the house front */}
          <mesh position={[0.45, 0.62, -1.62]} material={MAT.steelDark} castShadow>
            <boxGeometry args={[0.95, 0.8, 0.7]} />
          </mesh>

          {/* cab riser */}
          <RoundedBox args={[1.1, 0.5, 1.7]} radius={0.05} position={[-0.7, 0.47, -0.74]} material={MAT.paint} castShadow receiveShadow />
          <Cab />
          ...
          <group ref={boom} position={[0.45, 1.05, -1.55]}>
            <mesh geometry={geo.boom} material={MAT.paint} castShadow receiveShadow />
            <Decal text="CAT" logo position={[0.32, 1.28, -2.95]} rotation={[0, Math.PI / 2, 0]} width={1.1} />
            <Decal text="CAT" logo position={[-0.32, 1.28, -2.95]} rotation={[0, -Math.PI / 2, 0]} width={1.1} />
            {/* foot pin boss and stick pin boss */}
            <mesh rotation={[0, 0, Math.PI / 2]} material={MAT.steelDark}>
              <cylinderGeometry args={[0.2, 0.2, 0.72, 14]} />
            </mesh>
            <mesh position={[0, 0.26, -5.65]} rotation={[0, 0, Math.PI / 2]} material={MAT.steelDark}>
              <cylinderGeometry args={[0.17, 0.17, 0.68, 14]} />
            </mesh>
            {/* hydraulic lines along the boom top */}
            <mesh position={[0.12, 1.72, -2.9]} rotation={[Math.PI / 2, 0, 0]} material={MAT.black}>
            ...
            <group ref={stick} position={[0, 0.257, -5.65]}>
              <mesh geometry={geo.stick} material={MAT.paint} castShadow receiveShadow />
              <Anchor anchorRef={stickRod} position={[0, 0.6, 0.66]} />
              <Anchor anchorRef={bucketBase} position={[0, 0.5, -0.35]} />
              <mesh position={[0, 0, -2.9]} rotation={[0, 0, Math.PI / 2]} material={MAT.steelDark}>
                <cylinderGeometry args={[0.13, 0.13, 0.56, 12]} />
              </mesh>

              {/* ---------------- bucket ---------------- */}
              <group ref={bucket} position={[0, 0, -2.9]}>
                <mesh geometry={geo.shell} material={MAT.paintWorn} castShadow receiveShadow />
                <mesh geometry={geo.side} position={[0.6, 0, 0]} material={MAT.paintWorn} castShadow />
                <mesh geometry={geo.side} position={[-0.6, 0, 0]} material={MAT.paintWorn} castShadow />
                {/* linkage ear the bucket ram pulls on */}
                <mesh position={[0, 0.28, -0.1]} material={MAT.steelDark} castShadow>
                ...
                {[-0.48, -0.24, 0, 0.24, 0.48].map((x) => (
                  <mesh key={x} position={[x, -0.76, -1.0]} rotation={[0.25, 0, 0]} material={MAT.wear} castShadow>
                    <coneGeometry args={[0.07, 0.3, 4]} />
                  </mesh>
                ))}
                {/* the load */}
                <mesh ref={load} position={[0, -0.2, -0.55]} material={MAT.dirt} visible={false}>
...
function TrackLoop({
  ...
  return (
    <group position={[TRACK_GAUGE * side, 0, 0]}>
      ...
      <mesh position={[0, 0.05, 0]} material={shoes.bottom} receiveShadow castShadow>
      ...
      {[-1, 1].map((end) => (
        ...
        >
          <cylinderGeometry
            args={[IDLER_R + 0.05, IDLER_R + 0.05, TRACK_WIDTH, 16, 1, true, end > 0 ? -Math.PI / 2 : Math.PI / 2, Math.PI]}
          />
        </mesh>
      ))}
      {/* track frame */}
      ...
      <group ref={register} position={[0, IDLER_R + 0.05, -half]}>
        <mesh rotation={[0, 0, Math.PI / 2]} material={MAT.steel}>
          <cylinderGeometry args={[IDLER_R - 0.02, IDLER_R - 0.02, TRACK_WIDTH - 0.1, 18]} />
        </mesh>
      ...
      <group ref={register} position={[0, IDLER_R + 0.05, half]}>
        ...
        {Array.from({ length: 10 }, (_, i) => {
          ...
          return (
            <mesh key={i} position={[0, Math.sin(a) * (IDLER_R - 0.02), Math.cos(a) * (IDLER_R - 0.02)]} rotation={[a, 0, 0]} material={MAT.steel}>
              <boxGeometry args={[0.24, 0.08, 0.09]} />
            </mesh>
          );
        })}
        {/* final drive */}
        <mesh position={[side * 0.3, 0, 0]} rotation={[0, 0, Math.PI / 2]} material={MAT.black}>
      ...
      {/* carrier rollers */}
      {[-0.7, 0.7].map((z) => (
        <mesh key={z} position={[0, top - 0.08, z]} rotation={[0, 0, Math.PI / 2]} material={MAT.steel}>
...
function Cab() {
  ...
  return (
    <group position={[-0.7, 1.42, -0.72]}>
      ...
      <RoundedBox args={[1.08, 0.14, 1.66]} radius={0.05} position={[0, 0.76, -0.02]} material={MAT.paint} castShadow />
  ...
  return (
    <group>
      ...
      {[0, 0.5, 1].map((k) => {
        ...
        return (
          <mesh key={k} position={[p.x, p.y - posts / 2, p.z]} material={MAT.paint}>
