      ...
      <div className="grid grid-cols-[1fr_300px] gap-4">
            ...
            return (
              <li key={t.id}>
                <button
                  ...
                  aria-pressed={on}
                  className={cn("flex w-full items-center gap-4 rounded-2xl px-4 py-3.5 text-left transition", on ? "bg-white/8 ring-1 ring-[#FFC72C]/50" : "bg-white/3 hover:bg-white/6")}
                >
                  <span className="w-12 shrink-0 text-[14px] tabular-nums text-[#8E98A6]">{t.startsAt}</span>
                  {t.state === "active" ? (
                    <Ring value={t.progress / 100} size={40} stroke={4}>
                      <span className="text-[10px] tabular-nums">{Math.round(t.progress)}</span>
                    </Ring>
                  ) : (
                    <span className={cn("grid size-10 shrink-0 place-items-center rounded-full text-[13px]", t.state === "done" ? "bg-[#3DDC97]/15 text-[#3DDC97]" : "bg-white/5 text-[#8E98A6]")}>
                      {t.state === "done" ? <Check className="size-4" /> : i + 1}
                    </span>
                  )}
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-[16px] text-white">{t.title}</span>
                    <span className="text-[13px] text-[#8E98A6]">
                      {t.zone} · {t.soil} soil
                    </span>
                  </span>
                  <span className="text-right text-[15px] tabular-nums text-[#C4CBD4]">{t.state === "done" ? "Done" : fmtMin(t.state === "active" ? t.etaMinutes : t.totalMinutes)}</span>
                </button>
              </li>
            );
          })}
        ...
        {task ? (
          <Card title="How long, and why">
            ...
            <p className="mt-1 text-[13px] text-[#8E98A6]">Planner said {fmtMin(task.plannerMinutes)}</p>
            <ul className="mt-5 space-y-3">
              {task.drivers.map((d) => (
                <li key={d.feature}>
                  <div className="flex justify-between text-[14px]">
                    <span className="text-[#C4CBD4]">{d.label}</span>
      ...
      <div className="grid grid-cols-2 gap-4">
          ...
          <Card>
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-[13px] text-[#8E98A6]">{beltOff > 0 ? `${Math.round(beltOff)} s unbelted this shift` : "100% compliant this shift"}</p>
              </div>
            </div>
          </Card>
          <Card>
            <div className="flex items-center gap-5">
              ...
              <div>
                ...
                />
                <p className="mt-1 text-[13px] text-[#C4CBD4]">
        ...
        <Card
          ...
          action={
            <AnimatePresence>
              {logged ? (
                <motion.span initial={{ opacity: 0, y: 4 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="text-[13px] text-[#3DDC97]" role="status">
                  Logged
                </motion.span>
              ) : null}
            </AnimatePresence>
        ...
        >
          ...
          <textarea
            ...
            className="mt-3 w-full resize-none rounded-xl bg-white/5 p-3 text-[15px] text-white outline-none placeholder:text-[#566070] focus:ring-1 focus:ring-[#FFC72C]/60"
          />
          <Button variant="primary" icon={Send} onClick={submit} className="mt-3 w-full">
            Submit report
          </Button>
          <p className="mt-5 text-[12px] font-medium tracking-wide text-[#8E98A6]">Recent · auto and manual</p>
          <ul className="mt-2 space-y-2">
            {incidents.slice(0, 5).map((i) => (
              <li key={i.id} className="flex items-center gap-2.5 text-[14px]">
                ...
                <span className="min-w-0 flex-1 truncate text-[#C4CBD4]">{i.title}</span>
                <span className="shrink-0 tabular-nums text-[#566070]">{relativeTime(i.at)}</span>
              </li>
            ))}
          </ul>
        </Card>
    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4">
        <Card>
          ...
          <p className="mt-2 text-[13px] text-[#8E98A6]">{streak > 1 ? `Idling now · ${Math.round(streak)} s` : "Target under 15 min"}</p>
        </Card>
        <Card>
          <Stat label="Harsh events" value={<Num value={harsh} />} size="lg" tone={harsh > 3 ? "crit" : harsh > 0 ? "warn" : undefined} />
          <p className="mt-4 text-[13px] text-[#8E98A6]">Abrupt speed changes and snap reversals, measured live.</p>
        </Card>
        <Card>
          <Stat label="Fuel used" value={Math.round(fleet.fuelUsedL)} unit="L" size="lg" />
          <p className="mt-4 text-[13px] text-[#8E98A6]">
            {Math.round(fleet.loadCycles)} load cycles · {Math.round(fleet.utilization)}% utilised
          </p>
        </Card>
      </div>
      <p className="mb-3 mt-6 text-[13px] font-medium tracking-wide text-[#8E98A6]">Flagged by the anomaly model</p>
      <div className="space-y-3">
        {mine.length === 0 ? (
          <Card>
            <p className="flex items-center gap-2 text-[16px] text-[#3DDC97]">
          ...
          mine.slice(0, 3).map((a) => (
            <Card key={a.id}>
              <div className="flex items-baseline justify-between gap-3">
                ...
                <p className="text-[14px] tabular-nums text-[#FFB547]">₹{Math.round(a.costInr).toLocaleString("en-IN")}</p>
              </div>
              <p className="mt-1 text-[14px] text-[#8E98A6]">{a.explanation}</p>
              <p className="mt-3 rounded-xl bg-[#FFC72C]/8 px-4 py-2.5 text-[14px] text-[#F2E3B5]">{TIP[a.pattern] ?? TIP.unusual_pattern}</p>
            </Card>
          ))
        )}
      </div>
...
function Segmented<T extends string>({ label, options, value, onChange }: { label: string; options: T[]; value: T; onChange: (v: T) => void }) {
  return (
    <fieldset>
      <legend className="mb-2 text-[12px] font-medium tracking-wide text-[#8E98A6]">{label}</legend>
      <div className="relative flex rounded-xl bg-white/4 p-1">
        {options.map((o) => (
          <button key={o} onClick={() => onChange(o)} aria-pressed={value === o} className="relative z-10 h-10 flex-1 rounded-lg text-[14px] capitalize transition-colors" style={{ color: value === o ? "#14110a" : C.muted }}>
            {value === o ? <motion.span layoutId={`seg-${label}`} className="absolute inset-0 -z-10 rounded-lg bg-[#FFC72C]" transition={{ type: "spring", stiffness: 400, damping: 32 }} /> : null}
            {o}
          </button>
        ))}
      </div>
        ...
        <Card className="space-y-5">
          ...
          <div>
            <div className="flex items-baseline justify-between">
              <label htmlFor="est-volume" className="text-[12px] font-medium tracking-wide text-[#8E98A6]">
                Job size
              </label>
              <span className="text-[20px] font-light tabular-nums">{volume.toLocaleString()}</span>
            </div>
            <input id="est-volume" type="range" min={50} max={2000} step={50} value={volume} onChange={(e) => setVolume(Number(e.target.value))} className="mt-2 w-full accent-[#FFC72C]" />
          </div>
        </Card>
        <Card>
          <Label>Most likely</Label>
          <p className="mt-1 text-[52px] font-extralight leading-none tracking-tight text-[#FFC72C]">{fmtMin(est.p50)}</p>
          <p className="mt-2 text-[13px] text-[#8E98A6]">
            80% range {fmtMin(est.p10)} – {fmtMin(est.p90)}
          </p>
          <p className="mt-1 text-[13px] text-[#8E98A6]">Planner {fmtMin(est.plannerMin)}</p>
          <ul className="mt-5 space-y-2.5">
            {est.reasons.map((r) => (
              <li key={r.feature} className="flex justify-between text-[14px]">
                <span className="text-[#C4CBD4]">{r.label}</span>
...
function LessonPlayer({ title, onClose }: { title: string; onClose: () => void }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0 }} className="absolute inset-0 z-10 flex flex-col rounded-3xl bg-[#0b0e13] p-6" role="dialog" aria-label={title}>
...
export function TrainingApp({ onLaunchLesson }: AppProps) {
  ...
  return (
    <div className="relative min-h-full">
      ...
      <button onClick={onLaunchLesson} className="group relative mb-4 flex w-full items-center gap-5 overflow-hidden rounded-2xl bg-linear-to-r from-[#FFC72C] to-[#ffb020] p-5 text-left text-[#14110a]">
      ...
      <div className="grid grid-cols-[1.2fr_1fr] gap-4">
            ...
            {modules.slice(0, 4).map((m) => (
              <li key={m.id} className="flex items-center gap-3 rounded-xl px-3 py-2.5 hover:bg-white/3">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="text-[12px] text-[#8E98A6]">
        ...
        <Card title="Book an instructor">
          <div className="space-y-2">
            {days.map((day, di) => (
              <div key={day} className="flex items-center gap-2">
                <span className="w-24 shrink-0 text-[13px] text-[#8E98A6]">{day}</span>
          ...
          <p className="mt-4 flex items-center gap-2 text-[13px] text-[#C4CBD4]" role="status">
            <CalendarCheck className="size-4 text-[#8E98A6]" />
    ...
    <div>
      ...
      <div className="overflow-hidden rounded-2xl border border-white/6">
        <table className="w-full text-left text-[14px] tabular-nums">
          <thead className="bg-white/3 text-[12px] text-[#8E98A6]">
            <tr>
          ...
          <tbody>
            {machines.map((m) => {
              ...
              return (
                <tr key={m.id} className={cn("border-t border-white/5", m.id === fleet.id && "bg-[#FFC72C]/6")}>
                  <td className="px-3 py-2.5 text-white">{m.id}</td>
                  <td className="px-3 text-[#C4CBD4]">{m.operator?.name ?? "—"}</td>
                  <td className="px-3 text-[#C4CBD4]">{m.engineHours.toLocaleString()}</td>
                  <td className="px-3 text-[#C4CBD4]">{Math.round(m.fuelUsedL)} L</td>
                  <td className="px-3 text-[#C4CBD4]">{Math.round(m.loadCycles)}</td>
                  <td className="px-3" style={{ color: m.idleMinutes > 15 ? C.warn : "#C4CBD4" }}>
                    {Math.round(m.idleMinutes)}m
                  </td>
                  <td className="px-3">
                    <span className="flex items-center gap-2 text-[#C4CBD4]">
