...
const FAILED: Partial<Record<string, string>> = {
  denied:
    "Camera permission denied. Allow camera access for this site in the browser, then turn the camera off and on.",
  unavailable: "No camera available, or another app is using it.",
  error:
    "The vision model failed to load. Check the network connection and try again.",
};
  ...
  return (
    <div className="rounded-lg bg-white/4 px-2.5 py-2">
      ...
      <p
        ...
        style={{
          color: tone === "crit" ? C.crit : tone === "warn" ? C.warn : C.text,
        }}
      >
        {value}
      </p>
    ...
    <div className="grid gap-2">
      {!m.calibrated ? (
        <div className="rounded-lg bg-[#3BA9FF]/10 px-3 py-2 text-[12px] text-[#9fd2ff]">
          {m.face
            ? `Learning your eyes · look ahead normally · ${Math.round(m.calibration * 100)}%`
            : "Face the camera to calibrate"}
        </div>
      ) : null}
      <div className="grid grid-cols-3 gap-2">
        <Reading
          label="Eye openness"
          value={m.eyesClosed ? "Closed" : pct(m.eyeOpenness)}
          tone={m.eyesClosed ? "warn" : undefined}
        />
        <Reading
          label="Eyes closed"
          value={`${m.eyesClosedS.toFixed(1)} s`}
          tone={
            m.eyesClosedS >= T.MICROSLEEP_S
              ? "crit"
              : m.eyesClosedS >= T.LONG_BLINK_S
                ? "warn"
                : undefined
          }
        />
        <Reading
          ...
          tone={
            m.perclos !== null && m.perclos >= T.PERCLOS_DROWSY
              ? "crit"
              : m.perclos !== null && m.perclos >= T.PERCLOS_CLEAR
                ? "warn"
                : undefined
          }
        />
        <Reading
          label="Blinks / min"
          value={
            m.blinksPerMin === null ? "—" : String(Math.round(m.blinksPerMin))
          }
        />
        <Reading
          label="Long blinks 60 s"
          value={String(m.longBlinks)}
          tone={
            m.longBlinks >= T.LONG_BLINKS_DROWSY
              ? "crit"
              : m.longBlinks > 0
                ? "warn"
                : undefined
        ...
        <Reading
          label="Yawns 10 min"
          value={String(m.yawns)}
          tone={m.yawns >= T.YAWNS_FATIGUE ? "warn" : undefined}
        />
        <Reading
          label="Head turn"
          value={m.yawDeg === null ? "—" : `${Math.round(Math.abs(m.yawDeg))}°`}
          tone={
            m.yawDeg !== null && Math.abs(m.yawDeg) > T.DISTRACTED_YAW_DEG
              ? "warn"
              : undefined
          }
        />
        <Reading
          ...
          value={
            m.pitchDeg === null
              ? "—"
              : `${m.pitchDeg > 0 ? "↓" : "↑"} ${Math.round(Math.abs(m.pitchDeg))}°`
          }
          tone={
            m.pitchDeg !== null && m.pitchDeg > T.HEAD_DOWN_DEG
              ? "warn"
              : undefined
          }
        />
        <Reading
          label="Microsleeps 5 min"
          value={String(m.microsleeps)}
          tone={m.microsleeps > 0 ? "crit" : undefined}
        />
      </div>
    </div>
  );
    ...
    <div
      className={cn(
        compact
          ...
          : "grid grid-cols-[1fr_260px] items-start gap-4",
      )}
    >
        ...
        <div className="relative aspect-[4/3] overflow-hidden rounded-xl bg-black">
          {camera.enabled ? (
              ...
              {FAILED[camera.status] ? (
                <div className="absolute inset-0 grid place-items-center bg-black/70 px-6 text-center">
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <p className="text-[13px] text-[#C4CBD4]">
                      {FAILED[camera.status]}
                    </p>
                  </div>
                </div>
            ...
            <div className="absolute inset-0 grid place-items-center bg-[radial-gradient(circle_at_50%_40%,#1a2130,#07090d)]">
              <div className="flex flex-col items-center gap-2 text-center">
                {simulatedOnly ? (
                  <ScanFace className="size-8 text-[#FFB547]" aria-hidden />
                ) : (
                  <CameraOff className="size-7 text-[#566070]" aria-hidden />
                )}
                <p className="text-[13px] text-[#8E98A6]">
                  {simulatedOnly
                    ? "Simulated detection"
                    : "Operator camera off"}
                </p>
                {!compact ? (
                  <Button
                    icon={Camera}
                    variant="primary"
                    onClick={() => setCamera({ enabled: true })}
                    className="mt-2"
                  >
                    Turn on camera
                  </Button>
          ...
          <div className="absolute left-2.5 top-2.5 flex items-center gap-1.5 rounded-full bg-black/55 px-2.5 py-1 text-[11px] font-medium text-white backdrop-blur">
            <Dot
              ...
              pulse={camera.status === "running"}
            />
            {camera.status === "running"
              ? `Cab camera · ${camera.fps} fps`
              : camera.enabled
                ? camera.status
                : "Cab camera"}
          </div>
          {worst ? (
            <div
              ...
              style={{
                background:
                  worst.level === 3 ? C.crit : "rgba(255,181,71,0.92)",
                color: worst.level === 3 ? "#fff" : "#1a1305",
              }}
            >
              {worst.title}
            </div>
          ...
          {CAMERA_CHECKS.map((c) => {
            ...
            return (
              <li
                key={c.id}
                className={cn(
                  "flex items-center justify-between rounded-xl px-3.5 py-3 text-[14px]",
                  on ? "bg-status-crit/12" : "bg-white/4",
                )}
              >
                ...
                >
                  <Dot
                    tone={
                      on ? "crit" : camera.status === "running" ? "ok" : "off"
                    }
                  />
                  {on
                    ? camera.simulated[c.id]
                      ? "Simulated"
                      : "Detected"
                    : camera.status === "running"
                      ? "OK"
